...
/** 底部的工具栏 */
@property(nonatomic, weak) JWBottomToolView *toolView;
/** 顶部主播相关视图 */
...
bool _isSelected = NO;
- (JWBottomToolView *)toolView
{
    if (!_toolView) {
        JWBottomToolView *toolView = [[JWBottomToolView alloc] init];
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        _renderer.canvasMargin = UIEdgeInsetsMake(JWScreenHeight * 0.3, 10, 10, 10);
...
- (void)setRelateLive:(JWLive *)relateLive
{
...
//      2、用户http请求该地址，若请求成功表示直播未结束，否则结束
    ...
    [[JWNetworkTool shareTool] GET:self.live.flv parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
