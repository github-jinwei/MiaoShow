...
@interface JWNewStarViewController ()
/** 最新主播列表 */
...
@implementation JWNewStarViewController
...
- (void)setup
{
    ...
    // 设置header和footer
    self.collectionView.mj_header = [JWRefreshGifHeader headerWithRefreshingBlock:^{
        self.currentPage = 1;
...
- (void)getAnchorsList
{
    [[JWNetworkTool shareTool] GET:[NSString stringWithFormat:@"http://live.9158.com/Room/GetNewRoomOnline?page=%ld", self.currentPage] parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if ([statuMsg isEqualToString:@"fail"]) { // 数据已经加载完毕, 没有更多数据了
            ...
            NSArray *result = [JWUser mj_objectArrayWithKeyValuesArray:responseObject[@"data"][@"list"]];
            if (result.count) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (JWUser *user in self.anchors) {
        JWLive *live = [[JWLive alloc] init];
